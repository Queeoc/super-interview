...
> AI 面试平台，基于原 `biz_agent` 能力底座演进而来，支持面试流程、RAG 知识库问答和 MCP 工具接入
...
# 3. 准备配置文件
# 首次使用请先复制示例配置，再填写真实的 DASHSCOPE_API_KEY
cp .env.example .env
vim .env  # 或使用其他编辑器
...
# 3. 准备配置文件
# 使用记事本或其他编辑器先复制 .env.example 到 .env，再填写真实的 DASHSCOPE_API_KEY
...
├── logs/                                   # 日志目录（Loguru 自动创建，本地运行生成）
│   └── app_YYYY-MM-DD.log                  # 按天轮转的日志文件
├── uploads/                                # 上传文件临时目录（本地运行生成）
├── volumes/                                # Milvus 数据持久化目录（本地运行生成）
├── .env.example                            # 环境变量模板
├── .env                                    # 环境变量配置（需从模板复制后创建，勿提交）
...
> 说明：`uploads/` 和 `volumes/` 都是本地运行时目录，用于存放上传文件、Milvus/Postgres/Redis 等数据。请保留在本地，但不要提交到 GitHub。
